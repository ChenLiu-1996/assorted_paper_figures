...
if __name__ == '__main__':
    ...
    ax.barh(range(len(data_ablation_IEDB['raw_data'][0, ...].mean(axis=1))),
            data_ablation_IEDB['raw_data'][0, ...].mean(axis=1)[::-1],
            xerr=data_ablation_IEDB['raw_data'][0, ...].std(axis=1, ddof=1)[::-1],
            color=[(0.215686, 0.458824, 0.729412, alpha) for alpha in np.linspace(1.0, 0.2, 12)],
            ecolor='k',
            capsize=8,
    )
    df = pd.DataFrame(data_ablation_IEDB['raw_data'][0, ...].T[:, ::-1], columns=data_ablation_IEDB['ablations'])
    sns.swarmplot(ax=ax, data=df, size=6, color='#700000', marker="$\circ$", orient='h')
    ax.set_yticks(range(len(data_ablation_IEDB['ablations'])))
    ax.set_yticklabels(decode_ablation(data_ablation_IEDB)[::-1])
    ...
    ax.barh(range(len(data_ablation_IEDB['raw_data'][1, ...].mean(axis=1))),
            data_ablation_IEDB['raw_data'][1, ...].mean(axis=1)[::-1],
            xerr=data_ablation_IEDB['raw_data'][1, ...].std(axis=1, ddof=1)[::-1],
            color=[(0.215686, 0.458824, 0.729412, alpha) for alpha in np.linspace(1.0, 0.2, 12)],
            ecolor='k',
            capsize=8,
    )
    df = pd.DataFrame(data_ablation_IEDB['raw_data'][1, ...].T[:, ::-1], columns=data_ablation_IEDB['ablations'])
    sns.swarmplot(ax=ax, data=df, size=6, color='#700000', marker="$\circ$", orient='h')
    ax.set_yticks([])
    ax.set_xlim([0.4, 0.73])
    ...
    ax.barh(range(len(data_ablation_IEDB['raw_data'][2, ...].mean(axis=1))),
            data_ablation_IEDB['raw_data'][2, ...].mean(axis=1)[::-1],
            xerr=data_ablation_IEDB['raw_data'][2, ...].std(axis=1, ddof=1)[::-1] /
            np.sqrt(data_ablation_IEDB['raw_data'][2, ...].shape[1]),
            color=[(0.215686, 0.458824, 0.729412, alpha) for alpha in np.linspace(1.0, 0.2, 12)],
            ecolor='k',
            capsize=8,
    )
    df = pd.DataFrame(data_ablation_IEDB['raw_data'][2, ...].T[:, ::-1], columns=data_ablation_IEDB['ablations'])
    sns.swarmplot(ax=ax, data=df, size=6, color='#700000', marker="$\circ$", orient='h')
    ax.set_yticks([])
    ax.set_xlim([0.35, 0.55])
    ax.set_xticks([0.35, 0.42, 0.48, 0.55])
    ax.set_xticklabels([0.35, 0.42, 0.48, 0.55])
    ...
    items_shown = [7, 4, 0] # transfer + contrastive, transfer, none

    ax = fig.add_subplot(1, 4, 1)
    ax.bar(range(len(items_shown)),
           data_ablation_Cancer['raw_data'][0, ...].mean(axis=1)[items_shown],
           yerr=data_ablation_Cancer['raw_data'][0, ...].std(axis=1, ddof=1)[items_shown],
           capsize=20,
    ...
    handles, labels = ax.get_legend_handles_labels()
    df = pd.DataFrame(data_ablation_Cancer['raw_data'][0, ...][items_shown].T,
                      columns=np.array(data_ablation_Cancer['ablations'])[items_shown])
    sns.swarmplot(ax=ax, data=df, size=12, color='#700000', marker="$\circ$")
    ax.set_xticks([])
    ...
    ax = fig.add_subplot(1, 4, 2)
    ax.bar(range(len(items_shown)),
           data_ablation_Cancer['raw_data'][1, ...].mean(axis=1)[items_shown],
           yerr=data_ablation_Cancer['raw_data'][1, ...].std(axis=1, ddof=1)[items_shown],
           capsize=20,
           color=[(0.215686, 0.458824, 0.729412, alpha) for alpha in [1.0, 0.7, 0.4]])
    df = pd.DataFrame(data_ablation_Cancer['raw_data'][1, ...][items_shown].T,
                      columns=np.array(data_ablation_Cancer['ablations'])[items_shown])
    sns.swarmplot(ax=ax, data=df, size=12, color='#700000', marker="$\circ$")
    ax.set_xticks([])
    ...
    ax = fig.add_subplot(1, 4, 3)
    ax.bar(range(len(items_shown)),
           data_ablation_Cancer['raw_data'][2, ...].mean(axis=1)[items_shown],
           yerr=data_ablation_Cancer['raw_data'][2, ...].std(axis=1, ddof=1)[items_shown] /
           np.sqrt(data_ablation_Cancer['raw_data'][2, ...].shape[1]),
           capsize=20,
           color=[(0.215686, 0.458824, 0.729412, alpha) for alpha in [1.0, 0.7, 0.4]])
    df = pd.DataFrame(data_ablation_Cancer['raw_data'][2, ...][items_shown].T,
                      columns=np.array(data_ablation_Cancer['ablations'])[items_shown])
    sns.swarmplot(ax=ax, data=df, size=12, color='#700000', marker="$\circ$")
    ax.set_xticks([])
